...
def demo_pop_object() -> dict[str, Any]:
    return {
        "kind": "persona-object",
        "id": "persona:mentor:langchain-demo",
        "version": "1.0.0",
        "role": "mentor",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Demonstrate POPMiddleware with LangChain.")
    parser.add_argument(
        "--pop-file",
        help="Optional POP JSON file. If omitted, the script uses an inline demo object with a tool boundary.",
    )
    parser.add_argument(
        "--print-config",
        action="store_true",
        help="Print generated system prompt and tool filtering report without invoking a model.",
    )
    parser.add_argument(
        "--invoke",
        action="store_true",
